...
- `mr_reviewer` — revisa Merge Requests do GitLab via CLI `glab`, acionando o `analyzer` para julgar comentários inline e postando respostas/aprovações sob confirmação (skill: `review-mr`)
</subagents>
...
G1. **Acionar `analyzer`** com a skill `analyse-code` — completo
...
G9. **`code_reviewer`** com a skill `review-code` — corrigir críticos

G10. **`business_reviewer`** com a skill `review-code` — OBRIGATÓRIO; sem APROVADO/APROVADO COM RESSALVAS não versiona
...
G13. **`versioner`** com a skill `version-code` — somente com autorização explícita
</workflow>
