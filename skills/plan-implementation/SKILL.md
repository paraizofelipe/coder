...
### 2. Delegue ao `analyzer` (skill `analyse-code`)
...
### 3. Se houver ambiguidades, delegue ao `clarifier` (skill `clarify-intent`)
...
### 5. Delegue ao `planner` (skill `plan-tasks`)
...
### 6. Delegue ao `detailer` (skill `detail-tasks`)
