---
name: to-spec
description: Sintetiza decisões já tomadas (de `.coder/plan.md` ou da conversa atual) em um spec executável gravado em `.coder/spec-AAAAMMDD-HHMMSS.md`. Não entrevista — registra. Use depois da skill `planning`, quando o trabalho não cabe em uma única sessão e precisa sobreviver ao fim da janela de contexto.
---
...
1. `.coder/plan.md` (produzido pela skill `planning`) — fonte canônica quando existe
2. O contexto da conversa atual, quando não há `.coder/plan.md`
...
- Leia `.coder/plan.md` quando existir e mapeie suas seções com `references/plan-to-spec-map.md`. Quando não existir, sintetize do contexto da conversa.
...
- Não revalide fatos que o `.coder/plan.md` já confirmou.
...
<checklist>
- [ ] A base veio de `.coder/plan.md` ou de decisões efetivamente tomadas na conversa.
